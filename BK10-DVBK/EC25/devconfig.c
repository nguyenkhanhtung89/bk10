...
void load_device_setting(void){
	...
	//load sms password
	memset(SystemSetting.data.PassWord, 0x0, 16);
	...
	if(SystemSetting.data.OBDType > J1587_TYPE){
	...
	//load current week value
	DeviceSetting.CurrentWeek = read_number(CURRENT_WEEK_PATH);
	//load disable wlxtn
	DeviceSetting.DisableWLXTN = read_number(DISABLE_WLXTN_PATH);
	//load disable wlxtt
	DeviceSetting.DisableWLXTT = read_number(DISABLE_WLXTT_PATH);
	//load disable wcam
	DeviceSetting.DisableWCAM = read_number(DISABLE_WCAM_PATH);
	//load disable wrfid
	DeviceSetting.DisableWRFID = read_number(DISABLE_WRFID_PATH);
	...
	if(read_string(UPDATE_FW_URL_PATH, SystemSetting.data.update_fw_url) < 0){
		sprintf(SystemSetting.data.update_fw_url, "http://fw.dvbk.vn\0");
	}else{
	...
	if(read_string(UPLOAD_DB_URL_PATH, DeviceSetting.upload_db_url) < 0){
		sprintf(DeviceSetting.upload_db_url, "http://cam.dvbk.vn/api/UpFile/Device\0");
	}else{
		if((str_find(DeviceSetting.upload_db_url, ".") <= 2) || (strlen(DeviceSetting.upload_db_url) < 10)){
			sprintf(DeviceSetting.upload_db_url, "http://cam.dvbk.vn/api/UpFile/Device\0");
		}
	}
	strim(DeviceSetting.upload_db_url);
	...
	//load decrease speed mode
	DeviceSetting.Decrease_Speed = read_number(DECREASE_SPEED_PATH);
	if(DeviceSetting.Decrease_Speed > 1) DeviceSetting.Decrease_Speed = 0;
	...
	DeviceSetting.flg_clear_data_new_ver = read_number(FLG_CLEAR_NEW_VER_PATH);
