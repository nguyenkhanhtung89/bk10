...
void calculate_drive_data(void){
	...
	if(GPS_Data.gps_speed > SystemSetting.data.RunLevel){/* RUNNING... */
		...
		notify_time_cnt ++;
		if(DeviceSetting.DisableWRFID != 1){
		...
		if((tibase.DriverInWeek > (TIME_48_HOUR - SystemSetting.data.DrvForeWarningTime)) && ((DeviceSetting.DisableWLXTT != 1))){/* Check over driving in week time notify */
		...
		}else if((tibase.DriverInday > (TIME_10_HOUR - SystemSetting.data.DrvForeWarningTime)) && (DeviceSetting.DisableWLXTN != 1)){/* Check over driving in day time notify */
			if((notify_time_cnt % 19) == 0){
