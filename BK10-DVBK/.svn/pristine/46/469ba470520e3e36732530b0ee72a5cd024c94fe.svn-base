...
void serial_report_data(void){
	...
	/* Total km running from Pulse */
//	serial_data_rpt[16] = (ADCData.PulseDistance >> 24) & 0xFF;
//	serial_data_rpt[17] = (ADCData.PulseDistance >> 16) & 0xFF;
//	serial_data_rpt[18] = (ADCData.PulseDistance >> 8) & 0xFF;
//	serial_data_rpt[19] = (ADCData.PulseDistance & 0xFF);
...
static void EC21_data_parser(u8 * msg){
	...
	if(mcu_config.slience_mode != ec21_data.slience_mode){
		...
		FLASH_WriteData(SLIENCE_MODE_ADDR, (u32)mcu_config.slience_mode, 0);
	}
...
void Serial_Parse(uint8_t* data, uint16_t Len){//    >> cmd MSG CRC
	u8 cmd = 0;
	u8 crc = 0;
	//process data
	if(data[0] == '>'){    // >0x01 msg
