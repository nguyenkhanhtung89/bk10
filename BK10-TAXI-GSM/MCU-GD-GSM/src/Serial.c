...
//  }
	...
	/* load VIN NUMBER */
	memset(VIN_NUMBER, 0, 32);
	FLASH_ReadBufData(ADD_VIN_NUMBER, VIN_NUMBER, 17);
	if(strlen((char*)VIN_NUMBER) != 17){//not valid=> clear value
		memset(VIN_NUMBER, 0, 32);
		FLASH_WriteBufData(ADD_VIN_NUMBER, VIN_NUMBER, strlen((char*)VIN_NUMBER));
		flg_rcv_vin_number = 0;//reload again
	}
...
//  }
	...
	mcu_config.TypeOBD = VFE34_TYPE;//(enum_OBD_type)FLASH_ReadData(OBD_TYPE_ADDR);
	mcu_config.seat_level = FLASH_ReadData(TYPE_FULSE_ADC_ADDR);
	if((mcu_config.seat_level < 20) || (mcu_config.seat_level > 150)){
		mcu_config.seat_level = 35;
	}
}
...
void serial_report_data(void){
	...
	/* Speed ECU */
	serial_data_rpt[20] = canbus.connected | (ACC_STATUS << 1) | (/*DOOR_STATUS*/flg_rcv_vin_number << 2) | (AIR_STATUS << 3) | (PowerData.power_mode << 4) | sensorIR.status_seat << 5;
	
	/* Speed ECU */
	serial_data_rpt[21] = mcu_config.TypeOBD;
	
	/* Speed ECU */
	serial_data_rpt[22] = canbus.speedOBD;
	
	/* Temp ECU */
	serial_data_rpt[23] = canbus.seat_value & 0xFF;
...
void report_vin_number(void){
	uint8_t crc = 0;
	uint16_t tval = 0;
	memset(serial_data_rpt, 0x0, 64);
	
	serial_data_rpt[0] = '>';
	serial_data_rpt[1] = 12;
	
	memcpy(serial_data_rpt + 2, VIN_NUMBER, 17);
	
	crc = CRC_8BitsCompute(serial_data_rpt, 19);
	
	serial_data_rpt[19] = crc;
	
	SerialPushBuffer(serial_data_rpt, 20);
}
...
static void EC21_data_parser(uint8_t * msg){
	...
	ec21_data.seat_level = msg[3];//adcpulsetype
...
//	}
	...
	if(mcu_config.seat_level != ec21_data.seat_level){
		mcu_config.seat_level = ec21_data.seat_level;
		FLASH_WriteData(TYPE_FULSE_ADC_ADDR, (uint32_t)mcu_config.seat_level, 0);
	}
