...
int ec21_update_firmware(void){
	...
	if(flg_ec21_check_version == 1){
		...
		if(res == 0){//tai file version thanh cong
			...
			if(res < 2){
				log_system("->OPEN FILE version.txt ERROR<-\n");
				flg_ec21_check_version = 0;
			}else{
				log_system("->EC21 Server Ver: %s, Current Ver: %d<-\n", buffer, APP_VERSION);
				temp = ec21_atoi(buffer);
				if((APP_VERSION != temp) && (temp >= 10) && (temp < 255)){
					flg_ec21_update_fw = 1;

					log_system("->THERE ARE NEW UPDATE FOR EC21<-\n");
				}
				flg_ec21_check_version = 0;
			}
		}else if(res == -1){
			log_system("->DOWNLOAD version.txt ERROR<-\n");
			flg_ec21_check_version = 0;
			flg_ec21_update_fw = 0;
		}else if(res == -2){
			log_system("->wait internet and retry<-\n");
			//wait internet and retry
			sleep(1);
		}else if(res == -3){
			flg_ec21_check_version = 0;
			flg_ec21_update_fw = 0;
			log_system("file EC21 version.txt not found<-\n");
		}
		...
		if(res == 0){
			log_system("->DOWNLOAD EC21 FIRMWARE SUCCESS!!!<-\n");
			...
			if(res == 0){
				...
				if(res < 2){
					log_system("->EC21 crc.txt not exist or file error!!! <-\n");
					flg_ec21_update_fw = 0;
				}else{
					...
					if(ec_file_crc == ec_crc){
						...
						log_system("->ERROR EC21 crc cal: %u != file crc: %u !!! <-\n", ec_crc, ec_file_crc);
						flg_ec21_update_fw = 0;
					}
				}
				flg_ec21_update_fw = 0;
			}else{
				log_system("->UNZIP ERROR!!!<-\n");
				flg_ec21_update_fw = 0;
			}
		}else{
			flg_ec21_update_fw = 0;
			log_system("->DOWNLOAD EC21 FIRMWARE ERROR!!!<-\n");
		}
	}else{
		return 0;
	}
...
int set_mcu_update_fw(void){
	...
	if(res == 0){
		if(!strncmp(mcu_res, "OK", 2)){
			...
			log_system("FW UPDATE START OK\n");
			return 0;
		}else{
			log_system("FW UPDATE START ERROR\n");
		}
	}else{
		log_system("SEND COMMAND TO MCU ERROR, RES = %d\n", res);
	}
	return -1;
}
...
void mcu_update_firmware(void){
	...
	if(flg_mcu_check_version == 1){
			...
			if(mcu_send_and_rcv(">VERSION?", strlen(">VERSION?"), mcu_res, &mcu_res_len, 5000) == 0){
				if(mcu_res[0] == 'V'){
					...
					if(!strncmp(mcu_res, "GDBOOT", 6)){
						log_system("GD32 ALREADY IN BOOT MODE\n");
						//sprintf(MCU_Version, "BOOT");
						MCU_VERSION = 100;
						MCU_type = GD32_MCU;
					}else if(!strncmp(mcu_res, "BOOT", 4)){
						log_system("STM32 ALREADY IN BOOT MODE\n");
						//sprintf(MCU_Version, "BOOT");
						MCU_VERSION = 10;
						MCU_type = STM32_MCU;
					}else{
		...
		if(res == 0){//tai file version thanh cong
			...
			if(res < 2){
				log_system("READ FILE MCU version.txt ERROR\n");
				flg_mcu_check_version = 0;
			}else{
				log_system("MCU Server Ver: %s, Current Ver: %d\n", buffer, MCU_VERSION);
				temp = ec21_atoi(buffer);

				if((MCU_VERSION != temp) && (temp >= 10) && (temp <= 200) && (MCU_VERSION >= 10) && (MCU_VERSION <= 200)){
					flg_mcu_update_fw = 1;
					log_system("THERE ARE NEW UPDATE FOR MCU\n");
				}
				flg_mcu_check_version = 0;
			}
		}else if(res == -1){
			...
			flg_mcu_update_fw = 0;
			flg_mcu_check_version = 0;
			log_system("file MCU version.txt not found<-\n");
		}
		...
		if(flg_mcu_boot_mode == 1){
			...
			if(flg_downloaded_mcu_fw == 0){
				...
				if(res == 0){
					log_system("->DOWNLOAD MCU FIRMWARE SUCCESS!!!<-\n");
					...
					if(res == 0){
						...
						log_system("->MCU UNZIP SUCCESS!!<-\n");

						memset(buffer, 0x0, 256);
						res = read_text_file("/tmp/firmware/mcu/crc.txt", buffer);
						if(res < 2){
							log_system("->MCU crc.txt not exist or file error!!! <-\n");
							flg_mcu_update_fw = 0;
						}else{
							u32 mcu_crc = ec21_atoi(buffer);

							if(mcu_file_crc == mcu_crc){
								log_system("->MCU crc cal: %u, file crc: %u check success <-\n", mcu_crc, mcu_file_crc);
								flg_downloaded_mcu_fw = 1;
							}else{
								log_system("->ERROR MCU crc cal: %u != file crc: %u !!! <-\n", mcu_crc, mcu_file_crc);
								flg_mcu_update_fw = 0;
							}
						}
					}else{
					...
					flg_mcu_update_fw = 0;
					log_system("->DOWNLOAD MCU FIRMWARE ERROR!!!<-\n");
				}
		...
		if(update_fw_tmout >= 15){
			...
			if(mcu_send_and_rcv(">VERSION?", strlen(">VERSION?"), res, &res_len, 5000) == 0){
				if(!strncmp(res, "BOOT", 4)){
					log_system("MCU IN BOOT MODE > 15s => REUPDATE\n");
					set_mcu_update_fw();
				}else if(res[0] == 'V'){
					firmware_nPKG = 0;
					log_system(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>MCU UPDATE SUCCESS<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<\n");
