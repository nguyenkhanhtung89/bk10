...
char text[256];
static __IO u32 write_address = 0;
...
void disable_irqn(void){
	NVIC_DisableIRQ(USART_COM_IRQn);
...
char TEST_[128];
u8 num_1[4] = {0x0, 0x0, 0x0, 0x1};
u8 num_0[4] = {0x0, 0x0, 0x0, 0x0};
...
void FirmwarePackageParse(uint8_t *data){//> 0xFD 
	...
	/* Calculate CRC */
	cal_crc = calcCrc32((uint8_t *)data + 8, pkg_len);
	...
	if((pkg_id == fw_package_id) && (cal_crc == pkg_crc)){
		...
		if(WriteFwPackage(write_address, data + 8, pkg_len) != FLASH_COMPLETE){
			return;
		}
		
		cal_crc = calcCrc32((uint8_t *)write_address, pkg_len);
...
void FLASH_SetFLG(u32 addr, u32 state, uint8_t reset){
	/* Wait for last operation to be completed */
...
u32 FLASH_GetFLG(u32 addr){//set flg = 0 in app
	u32 Result = flash_read_u32(addr);
