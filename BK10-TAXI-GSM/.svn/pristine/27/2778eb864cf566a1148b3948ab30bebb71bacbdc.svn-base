...
void load_device_setting(void){
	...
	if((SystemSetting.data.filter_level < 20) || (SystemSetting.data.filter_level > 150)){
		SystemSetting.data.filter_level = 35;
	}
	...
	SystemSetting.data.DeltaSpeedPG = read_number(DELTA_SPD_PG_PATH);
	if((SystemSetting.data.DeltaSpeedPG < 5) || (SystemSetting.data.DeltaSpeedPG > 50)){
		SystemSetting.data.DeltaSpeedPG = 20;
	}
