...
void set_tz_gmt_hn(void){
	...
	tzset();
	log_debug("timezone=%d\n", timezone);
	log_debug("tzname[0]=%s, tzname[1]=%s\n",tzname[0], tzname[1]);
	log_debug("setenv %s ret = %d\n", stimezone, ret);
...
int file_download(char *link, char *dir ,char *name){
	...
	if(check_internet()){//check internet available
		...
		if(!check_url(link)){
			log_nw("the URL: %s AVAILABLE\n", link);
		}else{
			log_nw("the URL: %s is NOT AVAILABLE\n", link);
			return -3;
		}
