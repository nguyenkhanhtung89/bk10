...
static int callback_load_tibase(void *NotUsed, int argc, char **argv, char **azColName){
	...
	tibase.GPSMeter = atof(argv[14]);
	tibase.ti_hirring_sensor_state = (int16_t)(ec21_atoi(argv[15]));//tibase.temperature = (int16_t)(ec21_atoi(argv[15]));
...
static int callback_load_current_driver_data_by_UID(void *NotUsed, int argc, char **argv, char **azColName){
	...
	if(tibase.UID == _UID){
		...
		tibase.bMeter = tibase.GPSMeter - _METER;
	}
}
...
//	log_trace("\n");
	...
	Trip_Info.trip_hirring_sensor_status = (int16_t)(ec21_atoi(argv[P_PULSEDISTANCE]));//Trip_Info.temperature = (int16_t)(ec21_atoi(argv[P_PULSEDISTANCE]));
...
int init_database(void){
	...
	log_system("DRIVER INFO UPDATE 1\n");
...
int insert_TripInfo(trip_data_typedef GPSData){
	...
	sprintf(sql_cmd, "INSERT OR REPLACE INTO TripInfo VALUES ('%d', '%d', '%u', '%f', '%f', '%d', '%d', '%d', '%u', '%d', '%d', '%d', '%d', '%llu', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d')",
			...
			GPSData.trip_hirring_sensor_status, GPSData.fuel1, GPSData.fuel2, GPSData.fuel3, GPSData.tagUID, GPSData.GSMStreng, GPSData.main_power, GPSData.batery_power, GPSData.frqsend,
...
void send_backup_data(void){
	...
	*(float*)(buf.data + 23) = Trip_Info.location.longitude;
	...
	/* temp */
	buf.data[33] = Trip_Info.trip_hirring_sensor_status & 0xFF;
...
int update_tibase(tibase_typedef tib){
	...
	sprintf(sql_cmd, "INSERT OR REPLACE INTO TiBase VALUES (1, '%d', '%llu', '%f', '%f', '%f', '%f', '%u', '%u', '%u', '%d', '%d', '%d', '%d', '%f', '%d', '%u', '%u')\0",
			tib.LoginStatus, tib.UID, tib.b_loc_run.latitude, tib.b_loc_run.longitude, tib.b_loc_stop.latitude, tib.b_loc_stop.longitude, tib.bRunTime, tib.bStopTime, tib.RunSecondsPrvDay, tib.ParkCount, tib.OverSpeedCount, tib.DriveContinueCount, tib.OverDriveContinueCount, tib.GPSMeter, tib.ti_hirring_sensor_state, tib.DriverInday, tib.bMeter);
	//log_trace(sql_cmd);
...
char db_trace_name[128];
void upload_database(void){
	char str_cmd[160];

	sprintf(str_cmd,"curl -v -F filename=%s -F upload=@%s http://cam.dvbk.vn/api/UpFile/Device\0", db_trace_name, db_trace_name);
	system(str_cmd);
}

int copy_database(char* day){
	int rc;

	sprintf(db_trace_name, "/tmp/%llu-dbtrace.db", SystemSetting.data.DeviceID);

	if(access(db_trace_name, F_OK) == 0){
		unlink(db_trace_name);
	}

	rc = sqlite3_open(db_trace_name, &dbtrace);
	if(rc){
		log_system("Can't open dbtrace\n");
		return -1;
	}else{
		sem_wait(&db_sem);
		rc = sqlite3_open(DATA_BASE_PATH, &database);
		...
		if(rc){
			log_system("Can't open database\n");
			sqlite3_close(dbtrace);
			sem_post(&db_sem);
			return -1;
		}else{
			char *sql = "CREATE TABLE TripInfo ( \
					status1 INTEGER NOT NULL, \
					Status2 INTEGER NOT NULL, \
					uTime INTEGER NOT NULL, \
					Lat REAL NOT NULL, \
					...
					TripDay INTEGER NOT NULL, \
					TripHour INTEGER NOT NULL)";
			sqlite3_exec(dbtrace, sql, callback, (void*)data, &zErrMsg);

			sprintf(sql_cmd, "SELECT * FROM TripInfo WHERE TripDay = %s\0", day);

			rc = sqlite3_exec(database, sql_cmd, callback_copy_tripinfo, (void*)data, &zErrMsg);
			if(rc != SQLITE_OK){
			   log_nw("SQL error copy database: %s\n", zErrMsg);
			   sqlite3_free(zErrMsg);
			}
			sqlite3_close(database);
		}
