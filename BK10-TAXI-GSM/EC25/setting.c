...
static eNumCMDCode SetKeyActive(u8* msg){
	s32 len = 0;
	len = strlen(msg);
	if(len > 15) return INVALID_MESSAGE;
	msg[len] = 0;
...
static eNumCMDCode SetTaxiMode(u8* msg){
	u8 tmp = 0;
	if(str_find(msg, "ON") >= 0){
		tmp = 1;
	}else if(str_find(msg, "OFF") >= 0){
	...
	if(tmp == SystemSetting.data.taxi_mode) return ALREADY_SETTING;

	SystemSetting.data.taxi_mode = tmp;

	write_number(TAXI_MODE_PATH, (u64)SystemSetting.data.taxi_mode);

	return CMD_TAXI_MODE;
}

static eNumCMDCode SetHirring(u8* msg){
	u8 tmp = 0;
	if(str_find(msg, "ON") >= 0){
		tmp = 1;
	}else if(str_find(msg, "OFF") >= 0){
	...
	if(tmp == SystemSetting.data.hirring_ctrl) return ALREADY_SETTING;

	SystemSetting.data.hirring_ctrl = tmp;
...
static eNumCMDCode SetDeltaSpeedPG(u8* msg){
	u32 val = atoi(msg);

	if((val < 5) || (val > 50)) return INVALID_MESSAGE;

	if(val == SystemSetting.data.DeltaSpeedPG) return ALREADY_SETTING;

	/* store data */
	SystemSetting.data.DeltaSpeedPG = val;
	write_number(DELTA_SPD_PG_PATH, (u64)SystemSetting.data.DeltaSpeedPG);
	return CMD_DELTA_SPEED_PG;
}
...
static eNumCMDCode SetFilterLevel(u8* msg){
	...
	if((val < 20) || (val > 150)) return INVALID_MESSAGE;
...
static eNumCMDCode SetFirmwareUrl(u8* msg){
	s32 len = 0;
	len = strlen(msg);
	if(len >= MAX_SIZE_URL) return INVALID_MESSAGE;

	if((str_find(SystemSetting.data.update_fw_url, ".") <= 2) || (strlen(SystemSetting.data.update_fw_url) < 10)) return INVALID_MESSAGE;
...
static eNumCMDCode SetUploadFileUrl(u8* msg){
	...
	if((str_find(SystemSetting.data.upload_file_url, ".") <= 2) || (strlen(SystemSetting.data.upload_file_url) < 10)) return INVALID_MESSAGE;
...
void get_device_info(char* src){
	sprintf(src, "BK10-%d-%d-%d-%s\n%llu\n%s\n%s:%d\nGPS:%d, SVS: %d-%d\nTCP:%d, MODE:%d, STR:%d\nACC:%d-%d, D:%d, A:%d\nFRQ: %d-%d, MSP:%d\nV %d-%d-%d", APP_VERSION, MCU_Data.MCU_Verison, MCU_Data.RFID_Verison, RELEASE_DATE_TIME, SystemSetting.data.DeviceID, DeviceSetting.NumberPlate,
			...
			SystemSetting.data.FrqStop, DeviceSetting.SpeedLimit,MCU_Data.main_power, MCU_Data.Vout_power, MCU_Data.bat_power);
}
...
static void get_full_device_info(char* src){
	sprintf(src, "Device Type: BK10\nEC21: %d\nMCU: %d\nRelease: %s\nID: %llu\nPlate: %s\nIP/PORT: %s:%d\nTCP: %d, MODE: %d, STR: %d\nGPS: %d, SVS:%d-%d\nACC Hard: %d, ACC Sensor: %d, Door: %d, Air: %d\nFRQ Run-Stop: %d-%d, MaxSpeed: %d\nVin: %d, Vout: %d, Vbat: %d\nICCID: %s",
		...
		SystemSetting.data.FrqRun, SystemSetting.data.FrqStop, DeviceSetting.SpeedLimit,
...
static eNumCMDCode SetOBDType(u8* msg){
	...
	if(val != SystemSetting.data.OBDType){
		if(val > VF8_TYPE) return INVALID_MESSAGE;
		/* store data */
...
eNumCMDCode device_setting(u8* msg, SETTING_def type){//SET_04_121212
	...
	switch(cmd_code){
		...
		case CMD_SPEAKER_VOLUME://17
			...
			break;
		case CMD_DELTA_SPEED_PG://18
			cmd_code = SetDeltaSpeedPG(p1);
			break;
		...
		case CMD_PROVIDER://76
			...
			break;
		case CMD_TAXI_MODE://77
			cmd_code = SetTaxiMode(p1);
			break;
		case CMD_HIRRING:// 78
			cmd_code = SetHirring(p1);
			break;
...
char* get_device_setting(eNumCMDCode cmd, SETTING_def type){
	...
	switch(cmd){
		...
		case CMD_SPEAKER_VOLUME://17
			...
			return text;
		case CMD_DELTA_SPEED_PG://18
			sprintf(text, "Toc do cho phep chenh lech giua OBD vs GPS: %d", SystemSetting.data.DeltaSpeedPG);
			return text;
		case CMD_CHANGE_IP_CAMERA://19
		...
		case CMD_DRIVER_RESULT://54
			sprintf(text, "Thoi gian LXLT: %d(phut)\nThoi gian Dung: %d(phut)\nThoi gian LXTN: %d(phut)\nQuang duong: %0.02f(km)\nVuot toc: %d(lan)\nDung/Do: %d(lan)\nLXLT: %d(lan)\nVi pham LXLT: %d(lan)",
					(get_run_time() / 60), (get_stop_time() / 60), (tibase.DriverInday / 60), (tibase.GPSMeter / (double)1000.0), tibase.OverSpeedCount, tibase.ParkCount, tibase.DriveContinueCount, tibase.OverDriveContinueCount);
			return text;
		case CMD_FUEL_INFO://55
			sprintf(text,"Filter Level: %d\nGia tri cam bien nhien lieu:\nRS232 - 1 : %d\nRS232 - 2 : %d\nADC/PULSE : %d",SystemSetting.data.filter_level, MCU_Data.fuel_232_1, MCU_Data.fuel_232_2, MCU_Data.fuel_adc_in);
			return text;
		case CMD_CAM_CFG://56,
		...
		case CMD_GET_OBD_INFO://60
			sprintf(text,"SPEED -> %d, SEAT -> %d, RPM -> %d, PFUEL -> %d, S_KM -> %u, S_FUEL -> %d , BAT -> %d, AVG_ECON_FUEL-> %d, P_LOAD -> %d",
				MCU_Data.obd_data.sp, MCU_Data.obd_data.seat_value, MCU_Data.obd_data.rpm, MCU_Data.obd_data.p_fuel, MCU_Data.obd_data.s_km, MCU_Data.obd_data.s_fuel, MCU_Data.obd_data.bat, MCU_Data.obd_data.avg_econ_fuel, MCU_Data.obd_data.p_load);
			return text;
		case CMD_FWD_SMS:{//61
		...
		case CMD_OBD_TYPE:{//62
			switch(SystemSetting.data.OBDType){
				...
				case VFE34_TYPE:
					return "Giao thuc VFE34";
				case VF8_TYPE:
					return "Giao thuc VF8";
				default: break;
			}
			break;
		}
		...
		case CMD_PROVIDER://76
			...
			return text;
		case CMD_TAXI_MODE://77
			sprintf(text, "TAXI MODE: %s", SystemSetting.data.taxi_mode ? "ENABLE" : "DISABLE");
			return text;
		case CMD_HIRRING:// 78
			sprintf(text, "TAXI HIRRING STATUS: %s", SystemSetting.data.hirring_ctrl ? "ON" : "OFF");
			return text;
		case CMD_DISABLE_CAPTURE://87
