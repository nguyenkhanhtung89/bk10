...
void update_mcu_gps_data(void){
	...
	/* adc pull config */
	mcu_data[6] = SystemSetting.data.filter_level;//AdcPulseCfg -> filter_level
...
void mcu_report_data_parser(u8 * data){
	...
	MCU_Data.Hard_Door = ((data[18] >> 2) & 0x01);//((SystemSetting.data.DoorRevert == 1) ? !tmp0 : tmp0);
	...
	MCU_Data.seat_status = ((data[18] >> 5) & 0x01);

	MCU_Data.obd_data.typeOBD = data[19];

	MCU_Data.obd_data.sp = data[20];//SPEED

	MCU_Data.obd_data.seat_value = data[21];
...
//	log_system("SPEED -> %d, seat_value -> %d, RPM -> %d, Pfuel -> %d, s_km -> %u, s_fuel -> %d , bat -> %d, avg_econ_fuel-> %d, p_load -> %d",
...
int write_gps_data(void){
	...
	GPS.status_1 = GPS_Data.gps_status | (ACC_STATUS << 1) | (MCU_Data.hirring_local_state << 2) | (MCU_Data.hirring_sensor_state << 3) | (POWER_STATUS << 4) | (((tibase.LoginStatus == STATUS_LOGIN) ? 1 : 0) << 5) |((u8)tibase.over_speed_status << 6) | ((u8)tibase.over_driver_continue_status << 7);
	...
	*(float*)(buf.data + 22) = GPS_Data.location.longitude;
	...
	void send_realtime_data(void){
		...
		*(float*)(data_raw + 23) = GPS_Data.location.longitude;
		...
		data_raw[115] = MCU_Data.obd_data.seat_value;
...
//		hex_log_debug(buf.data, tcp_len);
	...
	void send_realtime_data(void){
		...
		/* Status of device  GPS - ACC - DOOR - AIR  - power mode (0 - nomal, 1 low pw) - LOGIN - OVSPEED - OV-LXLT */
		buf.data[13] = GPS_Data.gps_status | (ACC_STATUS << 1) | (MCU_Data.hirring_local_state << 2) | (MCU_Data.hirring_sensor_state << 3) | (POWER_STATUS << 4) | (((tibase.LoginStatus == STATUS_LOGIN) ? 1 : 0) << 5) |((u8)tibase.over_speed_status << 6) | ((u8)tibase.over_driver_continue_status << 7);
		...
		*(float*)(buf.data + 23) = GPS_Data.location.longitude;
		...
		/* cooling water temperature */
		buf.data[115] = MCU_Data.obd_data.seat_value;//seat_level
...
void send_device_setting(void){
	...
	buf.data[23] = SystemSetting.data.DeltaSpeedPG;//SystemSetting.data.SpeakerVolume;
	...
	tmp16 = MCU_Data.obd_data.seat_value;
...
//	buf.data[47] = (DeviceSetting.BauRate4 >> 24) & 0xFF;
	...
	buf.data[tcp_len] = tmp;
	tcp_len++;
	memcpy(buf.data + tcp_len, svs_info, tmp);//current_operator
	tcp_len += tmp;

	/* SIM IMSI current_operator */
	tmp = strlen(MCU_Data.VIN_NUMBER);
	buf.data[tcp_len] = tmp;
	tcp_len++;
	memcpy(buf.data + tcp_len, MCU_Data.VIN_NUMBER, tmp);
	tcp_len += tmp;
    ...
    switch(e_msg_id){
        ...
        case E_QL_LOC_NFY_MSG_ID_LOCATION_INFO:{
            ...
            if((ACC_STATUS == 1) || (GPS_Data.location.latitude == 0) || (GPS_Data.location.longitude == 0)){
            	int delta_speed = abs((int)GPS_Data.tmp_speed - (int)MCU_Data.obd_data.sp);
            	if(ACC_STATUS == 0){
            	...
            	if(((MCU_Data.obd_data.connected == 0) || (MCU_Data.MCU_Verison < 113)) || ((MCU_Data.obd_data.sp > 0) && (delta_speed <= SystemSetting.data.DeltaSpeedPG))){
			...
			// if(LAT != 0) GPS_Data.location.latitude = LAT;
			// if(LONG != 0)  GPS_Data.location.longitude = LONG;
	...
	for(;;){
		...
		if(STATE_GPS_READY != gps_init_state){
			...
			if((GPS_Data.tm_up > 0) && (GPS_Data.tm_up % 5) == 0){
				...
				update_mcu_gps_data();
				update_rfid_status();
				sprintf(svs_info, "%s-GPS:%d, view:%d, use: %d\0", current_operator, GPS_Data.gps_status, GPS_Data.svs_in_view, GPS_Data.svs_in_use);
				sprintf(ir_led_info, "Ir1:%d,Ir2:%d|IR:%d-%d|Seat:%d|HR:%d\0", MCU_Data.counter_ir_1, MCU_Data.counter_ir_2, MCU_Data.hirring_sensor_1, MCU_Data.hirring_sensor_2, MCU_Data.seat_status,  MCU_Data.hirring_local_state);
